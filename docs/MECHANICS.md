...
- **Switch and lever.** `Config.unrestEnabled()` (`GameRunner` registers the execution only when on; `conquer` records nothing when off); `balance:run --no-unrest` reproduces the previous build's hash exactly.

**Deliberately not built.** Garrison as troops rather than posts (the brief says "garrisoned"; a post is the one garrison the game has); partisans flipping land _directly_ to the former owner (they hold it as a tribe, and the former owner absorbs them by walking in); a shaded "occupied" map layer and an unrest readout on the player panel (`PlayerView.numUnrestTiles()` is there for it — Phase 4 item 6 work); nations garrisoning on purpose (the §05 7 personality work — today a nation's posts are wherever its build logic put them).
