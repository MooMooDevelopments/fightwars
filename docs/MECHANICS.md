...
- **There is no price effect** — an embargo is all-or-nothing; there is no tariff, no partial penalty, no route-weight reduction.
- Data-driven: no.
...
**There is no upkeep of any kind.** Verified by grep over `src/core` for `upkeep|Upkeep|maintenance` → zero hits. Specifically:

- Structures (`Structures` = City, DefensePost, SAMLauncher, MissileSilo, Port, Factory — `Game.ts:233-240`) cost gold once at `buildUnit` (`PlayerImpl.ts:1391`) and nothing per tick. `tests/economy/ConstructionGold.test.ts` explicitly asserts "charges gold once and no refund thereafter (allow passive income)".
- Warships cost `min(1,000,000, (n+1) × 250,000)` once (`Config.ts:538-545`), no per-tick cost. There is no refund on deletion (`DeleteUnitExecution` has no `addGold`).
- Troops have no cost to hold; the only per-tick troop _removal_ in the engine is the **Doomsday Clock drain** (`DoomsdayClockExecution.ts:162-170` → `m.removeTroops(doomsdayClockDrain(maxTroops, secondsPastWarn, cfg))`, curve in `src/core/game/DoomsdayClock.ts:334-357`, `drainStartPercent: 2 … drainMaxPercent: 5` of max per second), which is a punitive anti-stall mechanism, off by default (`DOOMSDAY_CLOCK_DEFAULTS.enabled = false`, `Config.ts:195-231`).
- `removeGold` clamps at zero (`PlayerImpl.ts:1332-1339`): the engine cannot represent debt, so any upkeep design must decide what happens at 0 gold.
...
#### G1. Supply lines / attrition far from City, Port or rail cluster

**Nothing exists.** There is no distance-to-anything term anywhere in `attackLogic` or in the tick loop; the only spatial query per tile is the 30-tile defense-post lookup.

**Injection points**

1. **Per-tile cost** is computed in exactly one place, `Config.attackLogic` (`Config.ts:843-934`), from one input struct `AttackLogicInput` (`:72-91`). Add a field (e.g. `supplyFactor: number` in [0,1] or `distToSupply: number`) to the struct, gather it in `AttackExecution.attackLogicInput` (`AttackExecution.ts:322-364`) next to `defenderHasDefensePost`, and multiply `mag` and/or `tileCost` right after the defense-post block (`:847-850`). Because `attackLogic` is pure, the golden test gives a reviewable diff.
2. **Attrition on the standing stack** (troops bleeding while far from supply, independent of conquest) belongs in the tick loop at `AttackExecution.ts:268-271` (before the budget loop): `troopCount -= attritionPerTick(...)` then `attack.setTroops`. Retreat returns whatever is left (§11), so attrition naturally punishes over-extension.
3. **Supply sources.** `RailNetworkImpl` already maintains connected components: `Cluster` (`src/core/game/TrainStation.ts:163-215`) holds `stations: Set<TrainStation>` and a private `tradeStations` (City/Port members, `:167-170`). `StationManagerImpl.findStation(unit)` (`RailNetworkImpl.ts:40-45`) maps a City/Port/Factory unit to its station; `station.getCluster()` gives the component; `RailNetwork.recomputeClusters()` (`RailNetwork.ts:13`) is already called on rail edits. Access from the sim: `game.railNetwork()` (`Game.ts:913`, `GameImpl.ts:1298`). For a cheap per-tile lookup use the existing spatial grid: `mg.hasUnitNearby(tile, R, UnitType.City|Port, attackerId)` (`GameImpl.ts:1107-1121`, Euclidean, active + built only) — same shape as the post check. Only two tiles' worth of extra queries per conquered tile. For "connected by rail" semantics, resolve the nearest attacker City/Port with `anyUnitNearby` + `findStation` + `getCluster()` and compare cluster identity with the source cluster.
4. **A per-tile "supplied" bit** can be stored in the free `state` bit 12 or the dead bit 14 (`GameMap.ts:137`, `setDefenseBonus`) and refreshed lazily; the renderer's R16UI texture already streams the state buffer, so the client could shade supplied/unsupplied territory with no new wire format.
5. **Terra nullius**: the brief's "far from supply" should also apply to the TN branch (`:857-869`), which currently has no distance term at all and floors at `5/(2·border)` per tile for stacks ≥ 6 600 troops.
...
#### 7.2 Materials as a construction gate

- Today the single gate is gold: `PlayerImpl.canBuildUnitType` (`1425-1440`: disabled? `_gold < cost`? alive?) used by both `canBuild` and `canUpgradeUnit`; the charge points are `buildUnit` (`1391`) and `upgradeUnit` (`1474`); the price surface is `UnitInfo.cost` returning `Gold` (`bigint`) and `BuildableUnit.cost/upgradeCosts` (`Game.ts:952-964`) shipped to the client through `PlayerActions`.
- Minimal pattern: add `materials?: (game, player, extraUnits?) => bigint` to `UnitInfo`; a `_materials` balance + `addMaterials/removeMaterials` on `PlayerImpl` mirroring `_gold`; check it in `canBuildUnitType` and charge in `buildUnit`/`upgradeUnit`; add `materialsCost` and `materialsUpgradeCosts` to `BuildableUnit` and the `PlayerView`/`PlayerUpdate` so the menu can grey out; production in `FactoryExecution.tick` (`FactoryExecution.ts:59-68` — currently does nothing after creating its station, so it is the natural producer, scaled by `factory.level()`), and optionally delivery via trains: `FactoryStopHandler.onStop` (`TrainStation.ts:219-225`) is an **empty hook** and `TrainExecution.loadCargo`/`Unit.setLoaded` (`127-136`, `UnitImpl.ts:751-756`) already exist with no caller, so "train picks up materials at a factory, delivers to a city" is mostly wiring.
- Deterministic maths: use `bigint` like gold; `DetMath` for any float.
...
- What exists: `Cluster.stations` gives every node tile (`station.tile()`); clusters are kept correct by `RecomputeRailClusterExecution` every tick after a removal, and merged eagerly on connect; `Railroad.tiles` gives the actual track tiles and `RailSpatialGrid.query(tile, r)` answers "is there track near here" in O(cells).
- Caveats to design around: (a) clusters are **cross-owner** — a cluster can contain stations of several players; supply must filter by `station.unit.owner()` (and possibly by `tradeAvailable` for allied transit, matching the brief's "rail transit" alliance tier). (b) Only City/Port/Factory are nodes; the brief's supply sources are "City, Port or rail cluster", so sources = own City/Port tiles ∪ own station tiles ∪ (optionally) own-owned track tiles. (c) `Cluster` is only touched on topology change, so a per-cluster cached "owned tile set" is cheap.
- Suggested shape: a `SupplyExecution` (or a field on `PlayerImpl` recomputed when `tileChangeVersion()` `1788-1790` or the rail topology changes) that floods from source tiles over the player's own territory — the flood in `validStructureSpawnTiles` (`1741-1761`, shared `tileTraversalScratch`) is the template — writing a per-tile supply distance; then extend `AttackLogicInput` (`Config.ts:72-91`) with `supplyDistance` and let `attackLogic` scale `mag`/`tickCost` (`843-934`) the way `defenderHasDefensePost` already does. That keeps the formula in `Config` (the balance surface) and the data collection in `AttackExecution`, matching the existing split.
...
#### A. Irradiated tiles producing nothing and costing extra to cross for minutes

- **What already exists**: crossing cost — `falloutDefenseModifier` multiplies both attacker losses and per-tile time cost by 3-5× (`Config.ts:851-855`). Production — nothing, but nothing is needed _yet_ because fallout tiles are structurally unownable (`GameImpl.setFallout` throws on owned tiles; `conquer` clears the bit at `GameImpl.ts:768`). The instant a tile is taken it is clean.
- **What is missing**: (1) a timer — fallout is permanent-until-conquered; (2) fallout on owned land; (3) any production hook.
- **Where to hook**:
  - Timer: there is no per-tile timestamp store. Options: a `Uint32Array falloutUntilTick` alongside `GameMapImpl.state` (bit 15 of the Uint16 is the only spare bit, insufficient), or an expiry queue in `GameImpl` drained once per second the way `DoomsdayClockExecution` gates on `ticks % 10`. `WaterManager.tick()` is the existing "deferred terrain mutation" pattern (`GameImpl.queueWaterConversion` → flush).
  - Owned fallout: relax `GameImpl.setFallout` (line 230-240) and stop `conquer` clearing it (line 768); `AttackExecution.ts:359` already passes `falloutRatio` whether or not the tile is owned, so the crossing penalty would then apply to defended irradiated land too (today the `defender !== null` path never sees fallout).
  - Production: `Config.maxTroops` (`Config.ts:985`) is the only tile-count-driven income (`numTilesOwned()^0.6`); subtract a per-player "irradiated owned tiles" count there (needs a counter on `PlayerImpl` maintained in `setFallout`/`conquer`/`relinquish`). Gold is flat per tick (`goldAdditionRate`, `Config.ts:1053`) — irradiation cannot reduce it via tiles; a multiplier would have to be added there or in `PlayerExecution.ts:86-88` where both rates are applied. Structures on fallout: `PlayerExecution.tick` (line 47-66) already deletes structures on unowned land; a "disabled while irradiated" check would go in `CityExecution`/`PortExecution`/`FactoryExecution` tick, or in `Config.maxTroops`' city-level sum (`filter(!isUnderConstruction)` at line 990-993 is the natural place for `&& !hasFallout(city.tile())`).
  - Attack multiplier: already parameterised — `AttackLogicInput.falloutRatio` (`Config.ts:87-88`) feeds `attackLogic`; changing `falloutDefenseModifier` or adding a `falloutAge` field is a one-file change plus `AttackExecution.ts:359`.

#### B. Global fallout escalation dropping everyone's regen past a threshold

- **Already flowing**: `numTilesWithFallout()/numLandTiles()` is computed per attacked tile in `AttackExecution.ts:359-361` and reaches `attackLogic` as `falloutRatio` — but only when the specific tile has fallout, and its effect is _inverted_ for the brief (more global fallout → cheaper crossing, modifier 5→3). It also silently shrinks the win and doomsday denominators (`WinCheckExecution.ts:130`, `DoomsdayClockExecution.ts:128`), which is a global escalation of sorts: every nuke makes the win bar easier to reach.
- **Missing**: no input to `troopIncreaseRate` (`Config.ts:1019-1050`), which is a pure function of `troops`, `maxTroops`, player type and difficulty.
- **Hook**: add a `falloutRatio` (or the whole `Game`) parameter to `troopIncreaseRate` — its only caller is `PlayerExecution.ts:86` which has `this.mg`; `GameImpl.totalLandTiles()` (`GameImpl.ts:981-983`) already returns `land - fallout`. A threshold curve of the form `toAdd *= 1 - k * max(0, ratio - threshold)` slots after the difficulty switch. Consider also making `falloutDefenseModifier` monotonic increasing if global fallout is meant to hurt, not help, attackers.

#### C. Extending the doomsday clock rather than duplicating it

- Everything is already factored for reuse: threshold math in `DoomsdayClock.ts` (`requiredBasisPoints`, `doomsdayClockRequiredTiles`, `doomsdayClockDrain`, `doomsdayClockTroopFloor`, `doomsdayClockRotQuota`, noise functions) is pure and integer-only; the per-side loop is in `DoomsdayClockExecution.tick`; player flags (`enterDoomsdayClock/clearDoomsdayClock/doomsdayClockTicks/markRotted`) are on `Player`; the HUD reads `doomsdayClockWaveState`.
- A "nuclear winter" trigger (fallout ratio ≥ X arms or accelerates the clock) belongs as a second input to `requiredBasisPoints` or as an `elapsed` offset computed in `DoomsdayClockExecution.tick` from `mg.numTilesWithFallout()` — e.g. `elapsed + falloutRatio * K` — so the HUD stays in sync for free. Rot already emits fallout (`consume`, line 352), so rot-as-nuclear-winter needs no new tile state. Add new tunables to `DOOMSDAY_CLOCK_DEFAULTS` and, if wire-configurable, to `DoomsdayClockConfigSchema` (`Schemas.ts:463`) and `doomsdayClockConfig()` (`Config.ts:281`). Do not create a parallel execution; the leader-exemption and side grouping logic must stay single-sourced.
...
- **Artillery**: `DefensePostExecution` is a land shell emitter whose ship-targeting block is commented out (`DefensePostExecution.ts:56-80`) and whose `defensePostTargettingRange() = 75` and `defensePostShellAttackRate() = 100` are already in Config — re-enable with a land-unit target list (attacks have no unit; artillery would need to target `AttackExecution` border tiles or structures, which `ShellExecution` cannot do today since it requires a `Unit` target).
- **Submarine**: a `WarshipState.state` variant or a `submerged` flag on `WarshipState` (`Game.ts:30-41`), excluded in `findBestTarget` (`WarshipExecution.ts:275-284`, the same place docked enemies are skipped) unless a detector is nearby; `nearbyUnits` type lists in `findTargetUnit`/`findRetreatAggroTarget` decide who can shoot at it.
- **Carrier**: port semantics are `owner.units(UnitType.Port)` lookups in `healWarship`, `findNearestPort`, `isPortFullOfHealing` (capacity = `port.level()`), `warshipSpawn`; a mobile carrier would need those to accept a `[Port, Carrier]` list and `getWaterComponent` of a moving tile.
- **Bomber**: `NukeExecution` already supports arbitrary `src`, `speed`, `waitTicks`, `rocketDirectionUp` and a caller-supplied magnitude via `nukeMagnitudes(type)`; a non-nuclear bomber is a new `NukeType` with small magnitude and a branch in `detonate` that skips `queueWaterConversion` (fallout). SAM whitelist must be extended in two places: `SAMTargetingSystem.getValidTargets` type list (`SAMLauncherExecution.ts:233`) and `SAMMissileExecution.nukesWhitelist` (`SAMMissileExecution.ts:45-49`), plus the `unitCount` fast path (`SAMLauncherExecution.ts:356-360`) and `SAMTargetingSystem.computeTargetScore` type bonus.

#### E. Radar stations extending SAM reach
...
#### 1. Tiered relations (NAP → Defensive Pact → Full Alliance → Vassal)

- Today's model is binary: `AllianceImpl{requestor, recipient, createdAt, expiresAt, two extension flags}` (`AllianceImpl.ts:4-18`) and `isFriendly = sameTeam || isAlliedWith` (`PlayerImpl.ts:1272-1280`). Every consumer of "can I hurt you" goes through `isFriendly`/`canAttackPlayer` (`AttackExecution.ts:85-95`, `PlayerImpl.ts:1893-1902`, `PlayerExecution.getCapturingPlayer :364`, nuke/MIRV cancellation, trade `canTrade`). Where to generalise:
  - Add a `tier` to `AllianceImpl` (and `AllianceRequestImpl` + `AllianceRequestIntentSchema`, `Schemas.ts:638-641`) and replace the single `isAlliedWith` with tier-aware predicates: NAP → `canAttackPlayer` false but donations/`assistAllies` off; Defensive Pact → `assistAllies`-style auto-retaliation from `findIncomingAttackPlayer`; Full → today's semantics; Vassal → one-directional (`requestor_`/`recipient_` already give you an asymmetric pair) plus a tribute hook next to `PlayerExecution.tick` gold add (`:88-89`) and `trainGold(rel)` (`Config.ts:449-`, which already switches on `"self"|"team"|"ally"|"other"` — the natural place for per-tier trade multipliers).
  - The `AllianceInfo` view (`PlayerImpl.ts:772-794`) and `toUpdate` alliances block (`:281-300`) are the client contract to extend.
  - Traitor logic (`GameImpl.breakAlliance :823-847`) needs a per-tier penalty table instead of the single 30 s flag; `traitorDefenseDebuff/SpeedDebuff` are the only two levers.
  - Nation acceptance (`NationAllianceBehavior.getAllianceDecision :88-148`) is a flat boolean chain; tiers would need it to return a max acceptable tier.

#### 2. War goals / peace terms

- Nothing exists. Attacks carry no intent beyond `AttackExecution{troops, target}`. The nearest hooks: `Stats.attack()` (`AttackExecution.ts:141`) records who attacked whom; `TargetPlayerExecution` is a public "declaration" that already propagates to allies via `transitiveTargets()`. A "war" object would sit alongside `AllianceRequestImpl` in `GameImpl.allianceRequests` (`:91`) with its own intent schema; peace = an alliance request of tier NAP with an expiry. Relation decay (`decayRelations`, 0.05/tick) already provides a "grudge fades" curve to hang terms on.

#### 3. Auto-coalitions at ~40% land

- No persisted leader. Reusable pieces: `WinCheckExecution.checkWinnerFFA/Team` sort by `numTilesOwned` every 10 ticks (`:84-111,166-178`) — the natural place to publish a "leader share" update; `percentageTilesOwnedToWin` gives the denominator convention (`numLandTiles - numTilesWithFallout`); `NationNukeBehavior.findFFACrownTarget` (`:295-360`) already computes `crownShare - myShare` with per-difficulty thresholds 0.4/0.3/0.2/0.1; `NationMIRVBehavior.selectVictoryDenialTarget` (`:154-198`) already does the team-aware ≥ 40% (Impossible) check. A coalition rule would: (a) add a `game.leader()`/`leaderShare()` on `GameImpl` computed in `WinCheck`; (b) in `getAllianceDecision`, accept any request whose requester is not the leader when `leaderShare > 0.4`; (c) in `maybeSendAllianceRequests`, target the leader's neighbours. `hasTooManyAlliances` (`:150-169`) exists specifically to keep enough enemies for the crown and would need inverting under a coalition.

#### 4. Persistent reputation as a lobby-join parameter

- `PlayerInfo` is built in exactly one place for humans: `GameRunner.createGameRunner` (`GameRunner.ts:50-61`) from `GameStartInfo.players[]` (`PlayerSchema`, `Schemas.ts:894-905`: clientID, username, clanTag, cosmetics, isLobbyCreator, friends, teamIndex). The server assembles that array in `GameServer.ts:1013-1027` from `Client` (`Client.ts`) and it is persisted into the replay record (`:1730-1750`, comment: every sim input must be in the record or replays desync). To add a badge without touching the sim: add an optional field to `PlayerSchema` and `PlayerRecord`, thread it into `PlayerInfo` (`Game.ts:436-464` constructor) as a new readonly, and read it only in client rendering (`PlayerView`) or in nation AI. If nations should read it (e.g. "distrust known betrayers"), it becomes a sim input and **must** be in the archived record. Nations already have a rudimentary in-game counter, `_betrayalCount`/`betrayals()` (`PlayerImpl.ts:865,909`), that nothing reads.
...
- A tribe is: `new PlayerInfo(name, PlayerType.Bot, null, random.nextID())` (`TribeSpawner.ts:82-87`) → `SpawnExecution(gameID, info, tile?)` → at first spawn `GameImpl.addPlayer` (`:667-679`; team = `botTeam` in team mode) + `PlayerExecution` + `TribeExecution` (`SpawnExecution.ts:59-64,99-104`). Spawning is allowed at any time (internal spawns are not phase-gated, `fromIntent=false`), and a spawn at a chosen `tile` conquers radius-4 tiles that are unowned (`getSpawnTiles(…, false)` filters owned tiles — a partisan spawn inside a player's territory would need `relinquish` first or a variant that conquers owned tiles).
- Bot stat profile lives entirely in `Config.ts` `PlayerType.Bot` branches (`:957-959,997-999,1027-1029,1056-1058,860,877-879,957`) and `attackLogic`'s `BOT_DEFENDER_LOSS_MULT = 0.7`. Bots: no gold (`startingGold 0n`, 50n/tick), no structures (they delete captured ones every 30 s), accept all alliances, `isOnSameTeam` always false among bots.
- `TribeExecution` decision loop (`:38-65`) is ~90 lines and self-contained on `AiAttackBehavior` (which works for bots because `allianceBehavior`/`emojiBehavior` are optional). A partisan = a tribe spawned by an "instability" execution (new `Execution` modelled on `DoomsdayClockExecution`, which already tracks per-player thresholds over ticks) with a target preference; `attackRandomTarget`'s traitor-first branch shows where to inject "attack the occupier".
- Names/themes: `resolveTribeNameData` gives themed prefix/suffix pools per map; partisans could reuse a dedicated theme key in `tribeNameThemes.json`.
- Ownership transfer on death: `GameImpl.conquerPlayer`/`PlayerExecution.removeCluster` (`:257-301`) handle enclave capture — surrounded partisans are absorbed by the enclosing player automatically.
...
**Speed:** there is **no game-speed setting**. The turn interval is a hard-coded 100 ms on both server and client (`src/server/ServerEnv.ts:108-110`, `src/client/ClientEnv.ts:135-137`), and `GameServerDeps.turnIntervalMs` is only injectable for tests (`GameServer.ts:129`, `:147-156`). The only speed controls are (a) `ReplaySpeedMultiplier {slow=2, normal=1, fast=0.5, fastest=0}` which multiplies the local turn interval in **singleplayer and replays only** (`src/client/utilities/ReplaySpeedMultiplier.ts`, `src/client/LocalServer.ts:36-40`, `:89-106`, driven by the `.`/`,` keys), and (b) `DoomsdayClockConfig.speed: slow|normal|fast|veryfast`, which is the anti-stall clock's pacing, not the sim rate (`Schemas.ts:463-466`). "Fast" as a lobby speed does not exist.
