...
**Radial menu on the keyboard (FightWars, session 13, brief §11 full keyboard navigation):** `M` opens the action menu on the tile at the screen's centre (`MainRadialMenu.handleOpenKey`, a synthetic `ContextMenuEvent`; ignored while typing in a field or with a modifier held). Inside the menu (`RadialMenu.handleKeyDown`, bound while it is visible): the arrows and Tab walk the current ring's enabled arcs in order, wrapping, skipping the disabled; Enter or Space does what a click does (`activate`, which the click shares); Escape and Backspace step back a level, Escape at the top closes. The focused arc is brightened with a two-pixel white ring and `aria-current`; the svg is `role="menu"`, each arc `role="menuitem"` with `aria-label` (the item's text or name) and `aria-disabled`, and a visually hidden `aria-live` region says the focused item's name. `tests/client/graphics/RadialMenuKeyboard.test.ts`: the walk with wrap and skip, Enter into a submenu and Escape out then closed, a leaf's action from Space, the roles and labels. Not done: keyboard reach for the centre button's own action (Enter with nothing focused), and the build and emoji sub-panels the menu opens, which have their own keyboard stories.
...
**Cold-load gate (FightWars, session 13, brief §11):** `npm run coldload:gate` (`scripts/coldLoadGate.ts`, the model in `scripts/coldLoadModel.ts`) runs in CI's build job after `build-prod`. It reads `static/index.html` through its template expressions, takes every script, stylesheet and module preload on the critical path, gzips each, and models the wire at 10 Mbps: 0.15 s to connect, 0.05 s a file, the bytes at line rate — 0.99 s for the current build (735 KB gzipped over four files), budget 2.5 s. It also sums the build's assets and the largest map (`channelislands`, 15.6 MB) as the initial download — 19.2 MB, budget 90 MB. Third-party scripts (CrazyGames, Turnstile) are outside the sum. What it does not model is JavaScript parse and execute on a 2019 laptop, which needs the laptop: this is the wire half of the number. `tests/scripts/ColdLoadModel.test.ts`.

**Map editor (FightWars, session 13, brief §6.9):** `src/core/game/MapPackage.ts` is the map format written a second time, in TypeScript — the Go generator (`map-generator/map_generator.go`) is the first. A painted grid (a terrain code and an elevation a tile) becomes the bytes `genTerrainFromBin` reads: one byte a tile row-major, bit 7 land, bit 6 shoreline, bit 5 ocean, bits 0–4 the magnitude (a land tile's elevation 0–30; an impassable tile `0b10011111`, not counted as land; a water tile half its distance to land, rounded up, capped at 31). Ocean is the largest four-connected water body — a lake is water that is not ocean; shoreline is land touching water and water touching land, never an impassable tile; `waterDistance` is a breadth-first search from every non-water tile. The two minimaps are the grid halved and halved again with water winning every contested cell (a river one tile wide survives the scaling) and impassable beating land, each scale's ocean, shoreline and distance recomputed rather than inherited. `buildMapPackage` returns the manifest and the three binaries; `validatePackage` is what the editor refuses on (no name, under 16×16, under 64 tiles of land, no nations, a nation off the map or at sea). `tests/MapPackage.test.ts` holds it to the byte and then hands the package to the real loader, which agrees about every tile. The editor itself is `<map-editor-modal>` (`src/client/MapEditorModal.ts`, the `map-editor` route, a page in `index.html`): a canvas one pixel a tile, five brushes with a size, three sizes to start from, nations placed by name with a click, and an export that downloads `<name>.fwmap.json` — the manifest and the three binaries base64'd into one file, `format: "fightwars-map/1"`. `gridFromPacked` opens a package back up — terrain and elevation from the bytes, the derived bits recomputed on the way out, so a package opened and exported again is byte-identical (a test pins that). **The community browser (same session):** `POST /maps` publishes an editor package (authenticated; `validatePackageFile` in `src/api/Maps.ts` re-checks it server-side — the format tag, each scale's byte count against its claimed dimensions, the land count against the bytes, the nations on land and inside the map, an 8 MB cap — because the editor is a client and a client is not a gate), `GET /maps?sort=new|rating&page=` lists them with the rating average and count, `GET /maps/:id` returns the package, `POST /maps/:id/rating` takes one to five stars, one per account, changeable (migration `0009_community_maps`: `community_maps` holding the package whole as JSONB, `community_map_ratings` keyed by map and account). `<map-browser-modal>` is the browser — newest or best rated, a star row that rates, and an Open that fetches the package and hands it to the editor; `<map-editor-modal>` gained Publish. Not done: playing a community map in a lobby (the lobby names its map by the `GameMapType` enum, so an arbitrary package needs a map identity on the wire, the server fetching it for the shadow sim, and every client fetching it too — a wire change, deliberately not attempted late in the session), a thumbnail, team spawn areas, and moderation of what is published.

**Challenges (FightWars, session 13, brief §6.7):** daily and weekly, read off the match records the API already ingests — nothing new crosses the wire from the game, and no client can claim progress it did not earn. `src/api/Challenges.ts` is the catalogue: each challenge is a period, a translation key, a target and a metric over one player's stats in one match (`won`, a count of games, troops sent from `ATTACK_INDEX_SENT`, structures from `OTHER_INDEX_BUILT`, gold from the trade and worker indices, warheads landed). `periodKey` is `d:YYYY-MM-DD` / `w:YYYY-Www` in UTC (ISO weeks, Monday to Sunday); `activeChallenges(at)` picks three dailies and two weeklies by hashing the period key, so the rotation is the same for every player and needs no state or deployment. `ingestMatch` adds each match's contribution per account (`challenge_progress`, migration `0008_challenges`: a row per account, period and challenge, with the completion stamped the first time the total reaches the target), and `/users/@me` carries `player.challenges` — id, period, name key, target, progress capped at the target, completed (`ChallengeProgressSchema`, appended and optional so the strict client parse keeps it). **Cosmetic only, by design:** a completed challenge is a line on a profile; the brief's "no pay-to-win, ever" holds, and nothing here touches a game. `tests/api/Challenges.test.ts`: the period keys across a midnight and a week boundary, the rotation's stability and its coverage of the catalogue over a month, one match's contribution read from its own stats, and three ingested games summing into the live set through the real API. The account modal's Challenges tab renders them (`<challenge-list>`: the two periods, each row a name, a bar and either the count or "Done"; `tests/client/ChallengeList.test.ts`). Not done: cosmetic rewards on completion (the catalogue is empty by design).

**Balance dashboard (FightWars, session 13, brief §11):** `/metrics` gained a balance table — per started game with a shadow, from `GameServer.balanceSnapshot()` (`GameManager.metrics()` carries it as `balance`): the tick, the count alive and the humans among them, the leader and their share of the claimable land (fallout excluded), the win bar of the moment (`percentageTilesOwnedToWin(elapsed)`), the share against the bar as a filled bar, the claimed share, and a sparkline of the leader's share over the page's last two minutes with the bar as a dashed line — one series a game, the action blue, the bar in the alert orange. Null without a shadow. `ShadowSimLike.game?()` is the read. `tests/server/BalanceMetrics.test.ts`.
