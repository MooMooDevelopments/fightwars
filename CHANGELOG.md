...
- **Map editor.** Paint your own map in the browser — water, plains, highland, mountain,
  impassable — place the nations that spawn on it, and export it as a map package.
...
- **The action menu works from the keyboard.** Press M to open it on the centre of the
  screen, arrows to choose, Enter to act, Escape to back out. Screen readers hear it.
...
- **Fixed:** a host changing game speed or any of the new modes after creating the lobby
  now changes the game, not just the host's screen.
- **Historical scenarios.** 1914, 1939, the Cold War and the Warring States: a fixed
  cast of nations on a real map, each with its doctrine and, where history had them, its
  bloc. Pick one in the host or solo lobby and join a side.
- **Placements.** Your first ten ranked games on a ladder are placement games: the
  ranked card counts them down instead of showing a rating, and you join the ladder once
  they are played.
- **Blitz in one click.** The speed picker in the host and solo lobbies has a Blitz card:
  4×, a compact map and five minutes on the clock, together.
...
- **The server now runs its own copy of every game.** A hacked client that asks to move
  ...
  saw, not the one the room voted for. A client that floods emoji, chat, alliance
  requests or donations is throttled at rates no hand reaches. Honest play is untouched.
...
- **Attack presets.** Shift+1 to Shift+4 set your attack ratio to a quarter, a half, three
  quarters or everything. Rebindable in Settings › Keybinds.
