...
export class GameInfoView extends LitElement {
    ...
    return html`
      ...
      >
        ...
        <button
          type="button"
          @click=${() => this.retry()}
          class="rounded-lg border border-action/40 bg-action/20 px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-action-ink transition-colors hover:border-action/60 hover:bg-action/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action-ink/70"
        >
    ...
    return html`
      ...
      >
        <div class="grid sm:grid-cols-[240px_minmax(0,1fr)]">
          <div
            class="relative min-h-36 overflow-hidden border-b border-white/10 bg-surface-deep sm:min-h-44 sm:border-b-0 sm:border-r"
          >
            ${this.mapImage
              ? html`<img
                  data-map-image
          ...
          <div class="flex min-w-0 flex-col justify-between p-4 sm:p-5">
            <div>
              <div class="mb-2 flex flex-wrap items-center gap-2">
                <span
                  class="rounded-md border border-action/25 bg-action/15 px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-action-ink"
                >
                  ${translateText(
              ...
              >
                <span class="text-action-ink/70" aria-hidden="true">
  ...
  private renderInfoMetric(
    ...
    return html`
      <div class="rounded-xl border border-white/[0.08] bg-black/15 p-2.5">
        ...
        >
          <span class="text-action-ink/70" aria-hidden="true">${icon}</span>
          ${label}
        </div>
  ...
  private renderMapFallback(): TemplateResult {
    return html`
      <div
        data-map-fallback
        class="absolute inset-0 flex items-center justify-center bg-[radial-gradient(circle_at_50%_40%,rgba(0,132,209,0.18),transparent_65%)] text-action-ink/35"
        aria-hidden="true"
      >
        <svg
          viewBox="0 0 24 24"
