...
export class GameView implements GameMap {
  ...
  public update(gu: GameUpdateViewData) {
    ...
    // Packed per-player stats: [smallID, tilesOwned, gold, troops,
    // goldEarned, materials] sextets for every player whose stats changed
    // this tick (the per-tick churn that no longer travels in PlayerUpdate
    // objects). Applied after pass 1 so first-emission players exist; their
    // sextet carries the same values as the full update, so double-applying
    // is harmless.
    const packedStats = gu.packedPlayerUpdates;
    if (packedStats !== undefined) {
      for (let i = 0; i + 5 < packedStats.length; i += 6) {
        ...
        state.materials = packedStats[i + 5];
      }
    }
