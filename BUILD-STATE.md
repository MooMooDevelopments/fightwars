...
- **What shipped.** Every scalar knob in `Config.ts` whose body was a plain number — 91 of
  them — now reads through one lookup against a registry (`Tunables.ts`: the number the
  code carries, bounds of zero to ten times it, integers rounded), and a lobby may carry
  a versioned ruleset of overrides on its config. Unknown keys are ignored, values
  clamped, and the ruleset rides the config every client and the shadow sim share, so
  lockstep holds. `resources/rulesets/default.json` is the registry as data, the
  versioned file of record, regenerated by `npm run rules:export` and held equal by a
  test. The host lobby got a Custom rules editor; the config patch carries the key; the
  join view counts the custom rules; the balance instrument takes `--ruleset`.
- **Kept out on purpose:** the turn's wall-clock length, the game speed, the bot count,
  the gold multiplier, the spawn-immunity pair, the spawn-phase and bot counts — timing
  and lobby settings, not balance — and every accessor with logic in it, which a mechanical
  rewrite must not touch.
- **The hash did not move.** The rewrite is a call through a map lookup that an empty
  ruleset skips; the baseline reproduced the caster commit's hash to the digit, and the
  perf gate's mean tick sits where it was.
- **A/B under a ruleset (the lever itself, 8000 ticks):**

  | metric (World, Medium, 150 bots, seed `perf-gate`) | defaults                                  | `--ruleset` supplyMaxRange 240, supplyMaxPenalty 0.1 |
  | -------------------------------------------------- | ----------------------------------------- | ---------------------------------------------------- |
  | alive at 8000                                      | 25                                        | 7                                                    |
  | top-1 / top-5 share                                | 13.9 % / 53.1 %                           | 51.1 % / 100 %                                       |
  | final hash                                         | `44138072306226350` (the caster commit's) | `41308704482894140`                                  |

  Read: doubling the supply reach and gutting the penalty lets the strongest nation
  run away with half the world by tick 8000 — the supply rule was holding the runaway
  in check, as §6.1 intended. That the instrument shows it from a two-line JSON file
  is the point of the lever.

- **Guards broken and watched fail:** the clamp dropped, the whitelist dropped.
- **Not done:** a form in place of the JSON box, hot reload mid-game (a ruleset is read
  when the game is built; a change is a new lobby), the accessors with logic as
  tunables, a ruleset picker for public lobbies.
